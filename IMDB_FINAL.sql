...
+---------------+-------------------+ */
-- Type your code below:
select genre,avg(duration) as avg_duration from movie inner join genre on movie.id = genre.movie_id group by genre order by avg(duration) desc
...
select title,avg_rating,DENSE_RANK() OVER (ORDER BY avg_rating desc) AS movie_rank from ratings inner join movie on movie.id = ratings.movie_id
...
select median_rating,count(movie_id) as movie_count from ratings group by median_rating order by  median_rating desc
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
select production_company,count(movie_id) as movie_count, dense_rank() over ( order by count(movie_id) desc)  from ratings r inner join movie m on m.id = r.movie_id 
...
where country like ('%USA%') and total_votes > 1000 and year = 2017 and month(date_published) = 3 group by genre order by movie_count desc
...
-- Type your code below:
select* from ratings
select name as actor_name, sum(total_votes) as total_votes,count(movie.id) as movie_count,avg(avg_rating) as actor_avg_rating,
...
+---------------+-------------------+---------------------+----------------------+-----------------+*/
-- Type your code below:
select name as actress_name, sum(total_votes) as total_votes,count(movie.id) as movie_count,avg(avg_rating) as actor_avg_rating,
rank() over(order by avg(avg_rating) desc ) as actor_rank
from names inner  join role_mapping on role_mapping.name_id = names.id 
inner  join movie on movie.id = role_mapping.movie_id
inner  join ratings  on ratings.movie_id = movie.id where country = 'India' and category =  'actress'
...
select name as actress_name,sum(total_votes) as total_votes,count(m.id) as movie_count, avg(avg_rating),
rank() over (order by avg(avg_rating) desc ) as actress_name from
movie m inner join ratings r on r.movie_id = m.id
